...
public class SensorReader : MonoBehaviour
{
    ...
    private IEnumerator InitConnection()
    {
        ...
        try
        {
            ...
            //OLD ACQUISITION SETUP
            /*
            if(serialNumber.Count > 0 && serialNumberAndType.Count > 0)
            {                
                connectionHandle1 = GdsClientApiLibraryWrapper.Connect(remoteEndpoint, localEndpoint, new string[] { serialNumber[0] }, true, out isCreator);
                connectionHandle2 = GdsClientApiLibraryWrapper.Connect(remoteEndpoint, localEndpoint, new string[] { serialNumber[1] }, true, out isCreator);

                //connect callback to get notified about errors in data acquisition
                GdsClientApiLibraryWrapper.SetDataAcquisitionErrorCallback(connectionHandle1, DataAcquisitionErrorCallback, IntPtr.Zero);
                GdsClientApiLibraryWrapper.SetDataAcquisitionErrorCallback(connectionHandle2, DataAcquisitionErrorCallback, IntPtr.Zero);

                //configure devices
                GUsbAmpGdsClientApiLibraryWrapper.GdsConfiguration[] deviceConfigurations = gTekConfigs.CreateConfiguration(serialNumber[0], serialNumberAndType[serialNumber[0]], out sampleRate, enabledChannels);
                GUsbAmpGdsClientApiLibraryWrapper.GdsConfiguration[] deviceConfigurations2 = gTekConfigs.CreateConfiguration(serialNumber[1], serialNumberAndType[serialNumber[1]], out sampleRate, enabledChannels);
                GdsClientApiLibraryWrapper.SetConfiguration(connectionHandle1, deviceConfigurations);
                GdsClientApiLibraryWrapper.SetConfiguration(connectionHandle2, deviceConfigurations2);

                //connect callback to trigger data processing each time when the specified amount of data is available
                GdsClientApiLibraryWrapper.SetDataReadyCallback(connectionHandle1, DataReadyCallback, (ulong)(DataReadyThresholdInMs / (double)1000 * sampleRate), IntPtr.Zero);
                GdsClientApiLibraryWrapper.SetDataReadyCallback(connectionHandle2, DataReadyCallback, (ulong)(DataReadyThresholdInMs / (double)1000 * sampleRate), IntPtr.Zero);

                //determine size of a single scan
                GdsClientApiLibraryWrapper.GetDataInfo(connectionHandle1, 1, out channelsPerDevice, out scanSizeInSamples);
                GdsClientApiLibraryWrapper.GetDataInfo(connectionHandle2, 1, out channelsPerDevice, out scanSizeInSamples);
                ...
                bufferSizeInSamples = bufferSizeInSeconds * sampleRate * scanSizeInSamples;
                receiveBuffer1 = new float[bufferSizeInSamples];           //(a float array of size 'bufferSizeInSamples' could be used here as well, but the BinaryWriter used later only supports writing arrays of type byte)
                receiveBuffer2 = new float[bufferSizeInSamples];           //(a float array of size 'bufferSizeInSamples' could be used here as well, but the BinaryWriter used later only supports writing arrays of type byte)
                //circBuffer = new Queue<float>((int)bufferSizeInSamples);
                //stack1 = new Stack<float>((int) bufferSizeInSamples);
                stack2 = new Stack<float>((int) bufferSizeInSamples);
                //for loop ist auskommentiert!
                for(int i = 0; i < (int)bufferSizeInSamples; i++)
                {
                    circBuffer.Enqueue(0f);
                }
    ...
    public void ContinueDeviceSetup()
    {
        ...
        // 3 - Pulse
        // 4 - Breathing
        if (uiHandel.firstDeviceType != 0 || uiHandel.secondDeviceType != 0)
        {
            //Setup first device:
            //Special case EEG
            //if the first device is of Type EEG
            if (uiHandel.firstDeviceType == 1)
            {
            ...
            if (uiHandel.firstDeviceType >= 2)
            {
                if (uiHandel.firstToggleDoubleUse.isOn == false)
                {
                    //only measure 8th Electrode
                    secondEnabledChannels = new bool[] {false, false, false, false, false, false, false, true};
                    writeTime.SetTypesAndUsage(doubleUseBool, uiHandel.firstDeviceType, 0);
                }
                else
                {
                    doubleUseBool = true;
                    secondEnabledChannels = new bool[] {false, false, false, false, false, false, true, true};
                    writeTime.SetTypesAndUsage(doubleUseBool, uiHandel.firstDeviceType, uiHandel.doubleDeviceType);
                }
            ...
            if (uiHandel.secondDeviceType == 1 && uiHandel.firstDeviceType != 1)
            {
                //measure 8 Electrodes
                firstEnabledChannels = new bool[] {true, true, true, true, true, true, true, true};
            ...
            if (uiHandel.secondDeviceType >= 2)
            {
                if (uiHandel.secondToggleDoubleUse.isOn == false)
                {
                    //only measure 8th Electrode
                    secondEnabledChannels = new bool[] {false, false, false, false, false, false, false, true};
                    writeTime.SetTypesAndUsage(doubleUseBool, uiHandel.secondDeviceType, 0);
                }
                else
                {
                    ...
                    doubleUseBool = true;
                    secondEnabledChannels = new bool[] {false, false, false, false, false, false, true, true};
                    writeTime.SetTypesAndUsage(doubleUseBool, uiHandel.secondDeviceType, uiHandel.doubleDeviceType);
                }
